...
UploadStatus azureBlob::uploadInBlob(const std::string& filename,
                                     const std::string& localPath)
{
    LOG_INFO("Starting upload for file: " + filename);
 
    std::string uploadURL = baseUrl + "/" + filename + sasToken;
    ...
    bool success = retryOperation([&]() {
        ...
        if (http_code >= 400 && http_code < 500) {
            LOG_WARN("Client error occurred. HTTP Code: " + std::to_string(http_code));
